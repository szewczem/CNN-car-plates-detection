...
batch_sizes = [32, 64]
num_of_dense_layers = [1, 2, 3]
output_in_dense = [4, 8, 16]
...
for i, (lr, batch_size, dense_config) in enumerate(product(learning_rates, batch_sizes, dense_layer_config)):
    ...
    early_stop = EarlyStopping(monitor='val_mean_iou', mode='max', patience=5, restore_best_weights=True, verbose=0)
...
print(f"\nBest IoU: {best_iou:.6f} with params: learning_rater: {best_params[0]}, batch_size: {best_params[1]}, dense_config: {best_params[2]}")

# Test model with the best selected parameters
test_model = load_model(model_params_file, custom_objects={'mean_iou': mean_iou})
test_loss, test_iou = test_model.evaluate(X_test, Y_test, verbose=0)
print(f"Test loss: {test_loss:.6f} | Test mean_iou: {test_iou:.6f}\n")

# Save the results with increased iou to .txt
